...
public class LottoGame02 {
    ...
    public static void main(String[] args) {
        ...
        // 즉 아래의 선언식의 경우
        // "int가 모여있는 int 배열 lottoNumbers를 선언하고 그리고 해당 배열의 크기(=길이)는 SIZE이다"
        // 라는 의미가 된다.
        ...
        // 그대신, 배열의 각 위치에는 해당 엘리먼트에 대한 주소값이 저장되어있다.
        ...
        // random.nextInt(NUMBER_MAX)+1 한 값을 할당해주는 for문이다.
        for (int i = 0; i < lottoNumbers.length; i++) { // int는 0~5까지
            // 아래 코드는
            // "lottoNumbers의 i번째 칸에 저장되어있는 주소값을 타고 가서
            // 그 공간에 random.nextInt(NUMBER_MAX) + 1 한 결과값을
            // 할당해라 라는 의미이다."
            lottoNumbers[i] = random.nextInt(NUMBER_MAX) + 1;
            // 0~44이기 때문에 +1을 해주어야 한다.
        }
        System.out.println();
        ...
        // 무식한 방법이다.
        // 만약 이 방법을 쓸 경우, j for문은 매우 많이 반복될 수도 있다!
...
//        }
        ...
        while (idx < lottoNumbers.length) {// 새로 추가해줄 숫자
            // 랜덤 객체를 사용해서 1~45 사이의 랜덤 숫자를 하나 뽑아서
            // randomNumber 변수에 할당한다!

            int randomNumber = random.nextInt(NUMBER_MAX) + 1;
            // randomNumber가 우리의 int 배열인 lottoNumber에 이미 존재하는 숫자일 때
            // 값이 false 로 바뀔 boolean변수 numberChecker
            ...
            // for문을 통해 lottoNumber의 각 엘리먼트가 randomNumber와 같은지 체크하여
