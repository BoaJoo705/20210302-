...
/*
 ...
 2. 로그인 후에 메인화면에서 학생 목록 보기를 들어가면 같은 반의
    교사와 학생의 정보들을 볼수 있다.
...
public class StudentManagement01 {
    ...
    // 교사 로그인 시 로그인한 학생의 정보를 저장할 Teacher 객체
    ...
    private static void showIndex() {
        while (true) {
            ...
            if (userChoice == 1) {
                // 로그인 메소드 실행 후
                // logIn 관련 객체를 검사하여 둘 중 하나가 null이 아닐 때에만
                // 메뉴를 보여주는 메소드 실행
                logIn();
    ...
    private static void setLogIn(int type, String username, String password) {
        if (type == 1) {
            ...
            Teacher temp = new Teacher();// 임시 객체는 왜 필요하지?..객체를 만들어서 통째로 넘기니까
            // 임시 객체에 username과 password 를 넣어준다.
            ...
            // username 과 password를 담을 student 임시 객체
    ...
    private static void register() {
        ...
        // 2. 로그인 시 사용할 아이디, 로그인시 사용할 비밀번호, 이름, 소속반을 받는다.
        // 단, 로그인시 사용할 아이디는 각각의 목록에서 중복이 없어야 한다.
    ...
    private static void printAll(ArrayList<Student> list) {
        while (true) {
            ...
            // 2-1. 입력한 값이 유효한지 확인하기 위해 id 가 userChoice인 Student 객체를 만든다.
            Student s = new Student();
            s.setId(userChoice);

            // 2-2. 사용자가 0을 입력하거나 혹은 유효한 번호를 입력했을 두 경우가 아닐 경우, 다시 입력을 받는다.
            // 유효한 번호란, 해당 소속반에 속한 학생의 번호와 일치하는 것이 존재하는 경우이다.
            ...
            // userChoice 가 0이 아닐 때에는 개별 보기 메소드를 호출해주고
            // userChoice 가 0일 때에는
            // break 한다.
            if (userChoice == 0) {
                break;
    ...
    private static void insertScore(int id) {
        ...
        // ScoreInputSwitch 필드의 값이 true 였는데 true로 바뀌게 된다.
        // 즉 변화가 없는 것이다.

        // 만약 이미 기존에 성적을 입력한 학생이었으면
        // scoreInputSwitch 필드의 값이 false 였는데 true 로 바뀌게 된다.
    ...
    private static void showNotgraded() {
        ...
        // 무한루프를 안에서 성적입력이 안된 학생들의 목록을 쭈르륵 뽑아준다.
        while (true) {
            for (Student s : list) {
    ...
    private static Student selectStudentById(int id) {// ->학생 아이디에 맞는 학생을 찾는
        Student s = new Student();
        s.setId(id);
