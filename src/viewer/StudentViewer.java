...
public class StudentViewer {
    ...
    // 1이면 모든학생 보여주기, 2이면 성적 입력 안된 학생 보여주기를 뜻하는
    // int 상수
    ...
    public void register() {
        ...
        // 1-4. 사용자가 입력한 아이디가 유효한 아이디인지 체크하고
        // 유효하지 않으면 다시 입력을 받는다.
        ...
        // 1-5. 미리 만들어놓은 학생 객체에 username을 추가해준다.
    ...
    // int type 의 경우 VALUE_ALL 이면 모든 학생 보여주기 이고, VALUE_NO_INPUT이면 아직 성적입력이 안된 학생으로
    // 생각하면
    // 된다.
        ...
        while (true) {
            ...
            if (logIn != null) {// *학생
                ...
                if (type == VALUE_ALL) {
                    ...
                    list = controller.selectByInputSwitch(group);// 성적입력안된 학생
                }
            }
            ...
            // 3-7. 만약 개별보기를 선택했으면 해당 메소드를 호출해주고
            // 아니면 break 한다.
            if (id == 0) {
                break;
            } else {
    ...
    public void printOne(int id) {
        ...
        String message;
        // 로그인이 null이면 1. 뒤로가기, 그외에는 1. 입력/수정 2. 뒤로가기 가 message의 값으로 정해진다.
        if (logIn == null) {
    ...
    // 현재 로그인 객체가 null이면 true, null이 아니면 false가 리턴되는 메소드
