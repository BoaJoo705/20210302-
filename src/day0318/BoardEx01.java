...
// 글번호를 자동으로 넣어줄때 사용될 int 변수
    ...
    private static void selectAll() {
        while (true) {
            ...
            // 사용자가 입력한 글의 id가 유효한 글의 id인지 체크하고, 유효하지 않으면 유효한 id나 0을 입력할 때까지
            // 다시 입력 받는다.

            // 글의 id가 유효한지 체크하기 위해서 Board 객체를 하나 만들어서, 그안에 사용자가 입력한 id 값을 입력한다(0이 아닐시)
            Board b = new Board();
            b.setId(userChoice);
    ...
    private static void selectOne(int id) {
        ...
        // Board 객체 b 에 파라미터로 넘어온 id값을 넣어준다.
        b.setId(id);
    ...
    private static void delete(int id) {
        ...
        // 2. index가 -1일 경우 해당 글 번호가 유요하지 않다는 의미이므로
        // 경고메시지만 출력하고 그 외에만 삭제를 진행한다.
        ...
        if (index == -1) {
            ...
            if (yesNo.equalsIgnoreCase("Y")) {
                ...
                // 3. 그 외에 다른 글자를 입력했을 경우, selectOne(id)를 실행한다.
                selectOne(id);
