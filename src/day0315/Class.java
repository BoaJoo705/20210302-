 ...
 * 우리 프로그램에서는
 * 우리가 필요한 클래스를 만들어서 그 클래스의 변수인 "객체를 만들어 
 ...
 * 호출되는 메소드이다.
 * 객체가 처음 초기화 될때
 * 필드의 초기값이나 특정한 행동을 해야할 경우, 우리가 직접 만들어줄수 있다.
 ...
 * 
 * java.lang.object안에는 기본생성자, toString(), equals() 등의
 ...
 * 오버라이드를 할 때 주의할 점은 부모클래스가 메소드를 선언한것과 똑같이 선언을 해주어야 한다는 것이다!!!!
 * 
 * 오버라이드 예시)
 * 1. equals
 ...
 *  해당 부모 클래스와 그 부모 클래스를 상속받는 모든 클래스가 그 자리에 대신할 수있는 성격을
 *  다형성이라고 한다.
