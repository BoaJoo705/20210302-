...
// 1. 접근제한자
//      접근제한자란, 해당 메소드가 어디에 있는 다른 클래스가 사용가능한지 규정하는 키워드이다.
...
//                  디폴트 접근 제한자라고도 하는데, 우리가 아무것도 적어주지 않으면 자동으로 패키지 접근제한이 걸린다.
//                  오히려, package 나 default 라고 적어주면 에러가 발생하는데, 그 이유는 해당 키워드들이 이미 다른곳에서
//                  사용되고 있기 때문이다!
...
// 3. 리턴 타입: 리턴 타입이란, 해당 메소드가 종료될때, 그 메소드를 호출했던 곳으로 보내줄 값의 데이터 타입을 리턴타입이라고 한다.
//              리턴 타입이 존재할 때에는 해당 메소드 내부에 반드시 return 이라는 키워드를 사용해서, 그 데이터 타입과 일치하는 데이터 타입을
...
// 4. 이름: 소문자로 시작하는 동사
// 5. 파라미터: 파라미터란, 해당 메소드를 실행 시킬때 외부로부터 꼭 받아와야 하는 측정한 종류의 데이터가 있을 경우
