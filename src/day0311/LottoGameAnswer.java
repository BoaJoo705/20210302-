...
public class LottoGameAnswer {
    ...
    public static void main(String[] args) {
        ...
        while (index < NUMBER_SIZE) {
            ...
            // boolean inputSwitch 를 만들고 true 로 초기화해준다.
            boolean inputSwitch = true;

            // for 문을 사용하여, randomNumber의 값과 computerArray 의 엘리먼트 중
        ...
        for (int i = 0; i < userArrays.length; i++) {
            ...
            if (userChoice == 1) {
                ...
                // index가 각게임 배열의 크기보다 작은 동안
                // index번 칸에 초기화해준다.
                while (index < userArrays[i].length) {
                    // 사용자한테 숫자를 선택 받아서
                    // 1~45 의 범위에 속하고 중복되지 않은 숫자인지 체크하여
                    ...
                    // 그외엔 false가 할당되는 booelean 변수 inputSwitch

                    // 이번 경우에는 inputSwitch의 초기화 값이
                    // 단순히 true가 아니라
                    // 사용자가 입력한 숫자가 1~45범위 안에 속하는지를
                    // 비교연산자와 논리연산자를 사용하여
                    // 그 결과값을 그대로 넣어주자
        ...
        // 2. 사용자의 각 배열 정렬하기
        for (int i = 0; i < userArrays.length; i++) {
            for (int j = 0; j < userArrays[j].length - 1; j++) {
